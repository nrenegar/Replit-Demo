...
def format_filename(term):
    clean_term = re.sub(r'[^\w\s]', '', term)  # Remove punctuation
    return f"/Users/nicholasrenegar/Library/CloudStorage/Dropbox/SHAPER Newsletter/Data/Recent Newsletter/{clean_term}.csv"

def fetch_and_save(term, paper_start_date):
    #while True:
    try:
        df_out = summarize_studies(term, paper_start_date)
        filename = format_filename(term)
        df_out.to_csv(filename, index=False)
        print(f"Data saved for {term}")
        #break
...
def combine_dataframes(directory):
    ...
    for file in files:
        df = pd.read_csv(file)
        print(f"Number of records in {file}: {df.shape[0]}")  # Prints the number of rows in the DataFrame
        dfs.append(df)
...
def get_authors(article):
    if 'AuthorList' in article:
        ...
        author_names = [f"{author.get('ForeName', '')} {author.get('LastName', '')}".strip() for author in authors]
...
def summarize_studies(search_term, paper_start_date):
    ...
    for start in range(0, 9998, chunk_size):
        ...
        print(f"Fetching records {start+1} to {start+chunk_size} of {total_records}")
        papers = entrez_utils.fetch_details(None, webenv, query_key, start, chunk_size)
        ...
        for paper in papers['PubmedArticle']:
            ...
            first_author_affil_list.append(get_first_author_affiliation(article))
...
def clean_journal_names(journal_name):
    ...
    journal_name = re.sub(r',\s*the$', 'The ', journal_name, flags=re.IGNORECASE)
